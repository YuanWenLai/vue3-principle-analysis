...
function createReactive(obj, isShallow = false, isReadonly = false) {
    return new Proxy(obj, {
        get(target, key, receiver) {
            console.log("🚀 ~ file: 5.8.1 代理set和map.js:334 ~ get ~ key:", key)

            // 5.8.1 通过指定第三个参数为原始对象，可以继续操作原有对象的方法和属性
            if(key === 'size') {
...
* @param {boolean} isShallow
* @return {void}
* @description createReactiveSetMap 函数,针对Set、Map
 */

// 5.8.2 针对Set、Map 监听对应的操作方法响应
const mutableInstrumentations = {
    add(key) {
        // this 指向的是代理对象，通过 raw 属性获取原始数据对象
        const target = this.raw
        // 避免重复设置响应，先判断值是否存在
        const hadKey = target.has(key)

        // 通过原始数据对象执行 add 方法，添加具体值
        const res = target.add(key)
        
        if(!hadKey) {
            trigger(target, key, TrrigerType.ADD)
        }
...
function createReactiveSetMap(obj, isShallow = false, isReadonly = false) {
    return new Proxy(obj, {
        get(target, key, receiver) {
            ...
            // 5.8.2 返回定义在mutableInstrumentations 对象下的方法
...
// 5.8.2 针对Set、Map
function reactiveSetMap(obj) {
    // 5.7.3 优先通过原始对象obj 寻找之前的代理对象
    const existionProxy = reactiveMap.get(obj)
    if(existionProxy) return existionProxy

    // 否则，创建新的代理对象
    const proxy = createReactiveSetMap(obj)
...
const p1 = reactiveSetMap(new Set([1,2,3]))

effect(() => {
    console.log(233, p1.size)
    console.log(233, p1)
})
